...
> **Node.js 18 LTS 이상 필요** — Ubuntu 20.04 기본 저장소의 `nodejs`는 버전이 너무 낮습니다.  
> 아래 NodeSource 명령으로 18 LTS를 먼저 설치하세요.
...
| 기능 수준 | 필요 패키지 |
|-----------|------------|
| 시리얼·레지스터·자동화 | `nodejs` (NodeSource 18+) |
| 패킷 캡처 (tcpdump) | `nodejs tcpdump` |
| 패킷 전송+캡처 (libpcap) | `nodejs libpcap-dev build-essential` |
...
```bash
# 1. Node.js 18 LTS 설치 (Ubuntu 20/22 공통)
curl -fsSL https://deb.nodesource.com/setup_18.x | sudo -E bash -
sudo apt-get install -y nodejs

# 2. 시스템 패키지
sudo apt-get install -y git tcpdump
# 선택: 패킷 전송도 필요하면 (cap npm 빌드용)
