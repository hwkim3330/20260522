...
async function readCountersFromSerial(serialBridge, portParam) {
  ...
  await new Promise((resolve) => {
    ...
    const tick = setInterval(() => {
      ...
      const done = Date.now() >= deadline || (counters.length > 0 && counters.length === lastCount);
      lastCount = counters.length;
      if (done) { clearInterval(tick); serialBridge.events.off('serial', onEvent); resolve(); }
    }, 200);

    setTimeout(() => { clearInterval(tick); serialBridge.events.off('serial', onEvent); resolve(); }, 8500);
  });
...
router.get('/counter/read', async (req, res) => {
  ...
  try {
    const counters = await readCountersFromSerial(req.app.locals.serialBridge, portParam);
